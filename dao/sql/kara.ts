...
export const sqlRefreshSortableTable = (
	selectClauses: string[],
	joinClauses: string[],
	whereClauses: string[]
) => `
SELECT k.pk_kid AS fk_kid,
	(select d.list
		from kara k2
		CROSS JOIN LATERAL (
		...
		) d WHERE k2.pk_kid = k.pk_kid) AS titles,
	${selectClauses.join(',\n')}
FROM kara k
...
create index IF NOT EXISTS idx_ak_search_vector
    on all_karas using gin (search_vector);

create index IF NOT EXISTS idx_ak_created
    on all_karas (created_at desc);

create index IF NOT EXISTS idx_ak_songorder
    on all_karas (songorder);

create index IF NOT EXISTS idx_ak_year
    on all_karas (year);

create UNIQUE index IF NOT EXISTS idx_ak_kid
    on all_karas (pk_kid);

create index IF NOT EXISTS idx_ak_search_vector_parents
	on all_karas using gin (search_vector_parents);

create index IF NOT EXISTS idx_ak_anilist
	on all_karas (anilist_ids);

create index IF NOT EXISTS idx_ak_kitsu
	on all_karas (kitsu_ids);

create index IF NOT EXISTS idx_ak_myanimelist
