...
export async function gitlabPostNewIssue(
	title: string,
	desc: string,
	labels: string[]
): Promise<string> {
	...
	const res = await HTTP.post(
		`${conf.Gitlab.Host}/api/v4/projects/${
			conf.Gitlab.ProjectID
...
export async function postSuggestionToKaraBase(
	title: string,
	serie: string,
	type: string,
	link: string,
	...
	try {
		...
		sentry.error(err);
		logger.error('Call to Gitlab API failed', {
			service: 'KaraSuggestion',
			obj: err,
		});
	}
}
