...
async function moveKaraToImport(
	...
	// Let's move baby.
	if (sourceMediaFile)
		await copy(sourceMediaFile, resolve(resolvedPath('Import'), newMediaFile), {
			overwrite: true,
		});
	if (kara.subfile)
		await copy(sourceSubFile, resolve(resolvedPath('Import'), newSubFile), {
			overwrite: true,
		});
	return {
		lyrics: newSubFile,
...
async function cleanupImport(importFiles: ImportedFiles) {
	if (importFiles?.media)
		await fs.unlink(resolve(resolvedPath('Import'), importFiles.media));
	if (importFiles?.lyrics)
		await fs.unlink(resolve(resolvedPath('Import'), importFiles.lyrics));
}
		...
		if (!tag.tid) {
			...
			if (y > -1 && allTags[y].tid) {
				...
				await editTag(
					...
					{ silent: false, refresh: false, repoCheck: true, writeFile: true }
				);
			}
			if (y > -1 && !allTags[y].tid) {
