...
export async function getMediaInfo(mediafile: string, computeLoudnorm = true): Promise<MediaInfo> {
	try {
		...
		if (supportedFiles.audio.some(extension => mediafile.endsWith(extension))) mediaType = 'audio';
		else if (supportedFiles.video.some(extension => mediafile.endsWith(extension))) mediaType = 'video';
		else {
